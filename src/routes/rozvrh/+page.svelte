<main>
    <section class="pt-10 pb-20 bg-black text-white">
        <!-- Nadpis sekce -->
        <div class="container mx-auto text-left mb-16 font-bold">
            <h2 class="text-3xl font-bold font-heading mb-5 text-center">Skupinové lekce s trenéry</h2>
            <div class="flex flex-col gap-6  px-4 md:px-8 max-w-4xl">
                <p class="text-lg text-gray-400">
                    Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
                </p>

                <p class="text-lg text-gray-300">
                    <span class="text-yellow">Podmínkou</span> konání lekce jsou <span class="text-yellow">minimálně</span> 2 přihlášení účastníci
                </p>

                <p class="text-lg text-gray-300">
                    Účast na lekci rezervujte <span class="text-yellow">online</span> nebo na <span class="text-yellow">info@beefit.cz</span> / <span class="text-yellow">733 60 60 02</span>
                </p>
            </div>

            <!-- Tlačítko pro rezervace -->
            <div class="text-center mt-6">
                <button class="bg-yellow text-black font-bold py-2 px-8 rounded">
                    Online rezervace
                </button>
            </div>
        </div>

        <!-- Rozvrh lekcí -->
        <div class="container mx-auto mt-12 px-4 font-bold">
            <div>
                <h3 class="text-2xl text-yellow mb-4">Úterý</h3>
                <ul class="list-disc list-inside text-gray-300 space-y-2">
                    <li>10:00 - 11:00</li>
                    <li>Pružné tělo s Martinem</li>
                    <li>Lekce zaměřena na problémy sedavého zaměstnání</li>
                </ul>
            </div>

            <div class="mt-12">
                <h3 class="text-2xl text-yellow mb-4">Středa</h3>
                <ul class="list-disc list-inside text-gray-300 space-y-2">
                    <li>10:00 - 11:00</li>
                    <li>Pružné tělo s Martinem</li>
                    <li>Lekce zaměřena na problémy sedavého zaměstnání</li>
                </ul>
            </div>

            <div class="mt-12">
                <h3 class="text-2xl text-yellow mb-4">Čtvrtek</h3>
                <ul class="list-disc list-inside text-gray-300 space-y-2">
                    <li>10:00 - 11:00</li>
                    <li>Pružné tělo s Martinem</li>
                    <li>Lekce zaměřena na problémy sedavého zaměstnání</li>
                </ul>
            </div>
        </div>
    </section>
</main>
